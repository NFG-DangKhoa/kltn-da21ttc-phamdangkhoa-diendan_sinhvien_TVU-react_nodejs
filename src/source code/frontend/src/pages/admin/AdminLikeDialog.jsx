...
const AdminLikeDialog = ({ open, onClose, postId }) => {
    ...
    const fetchLikedUsers = async () => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:5000/api/admin/likes/post/${postId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });

            const users = response.data.data.map(like => like.userId).filter(user => user);
            setLikedUsers(users);
        } catch (error) {
...
export default AdminLikeDialog;
