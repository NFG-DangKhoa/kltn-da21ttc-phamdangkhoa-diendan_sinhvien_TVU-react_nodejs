    ...
    useEffect(() => {
        const loadAdminInfo = async () => {
            try {
                ...
                const contactResponse = await axios.get('http://localhost:5000/api/admin/contact-info');
                if (contactResponse.data.success) {
                    setAdminInfo(contactResponse.data.data);
                } else {
                    // Fallback: get first admin user
                    const usersResponse = await axios.get('http://localhost:5000/api/users?role=admin&limit=1');
                    if (usersResponse.data.success && usersResponse.data.users.length > 0) {
                        const admin = usersResponse.data.users[0];
                        setAdminInfo({
                            name: admin.fullName || 'Admin TVU Forum',
                            title: 'Quản trị viên diễn đàn',
                            email: admin.email || 'admin@tvu.edu.vn',
                            phone: admin.phone || '0123-456-789',
                            address: admin.address || 'Trường Đại học Trà Vinh, Trà Vinh',
                            description: admin.bio || 'Chúng tôi luôn sẵn sàng hỗ trợ và giải đáp mọi thắc mắc của bạn. Hãy liên hệ với chúng tôi khi cần thiết!',
                            avatar: admin.avatarUrl,
                            socialLinks: {
                                facebook: admin.facebook || 'https://facebook.com/tvu.edu.vn',
                                website: admin.website || 'https://tvu.edu.vn',
                                email: admin.email || 'admin@tvu.edu.vn'
                            },
                        ...
                        <StyledCard>
                            <CardContent sx={{ p: 4 }}>
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 4 }}>
                                    <Avatar
                                        src={constructUrl(adminInfo.avatar)}
                                        sx={{
                                            width: 120,
                                            height: 120,
                                            mb: 2,
